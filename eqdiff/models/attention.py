...
class Transformer3DModel(ModelMixin, ConfigMixin):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, timestep=None, return_dict: bool = True):
        ...
        # Blocks
        #print(f'in attention:{video_length}')
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        self.use_our = use_our

        # SC-Attn
        assert unet_use_cross_frame_attention is not None
        if unet_use_cross_frame_attention:
            print(f'using cross frame attention.')
            self.attn1 = SparseConditionAttention(
                query_dim=dim,
        ...
        if not is_xformers_available():
            ...
            if self.use_our:
                if self.attn_cond is not None:
                    self.attn_cond._use_memory_efficient_attention_xformers = use_memory_efficient_attention_xformers
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, timestep=None, attention_mask=None, video_length=None):
        ...
        #    )
        #else:
        #    #print(f'norm_hidden_states:{norm_hidden_states.shape}, video_length:{video_length}')
        #    hidden_states = self.attn1(norm_hidden_states, attention_mask=attention_mask, video_length=video_length) + hidden_states
...
class SparseConditionAttention(CrossAttention):
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, video_length=None, use_temp=True):
        batch_size, sequence_length, _ = hidden_states.shape
        ...
        key = torch.cat([key[:,  [0] * video_length], key[:,  [1] * video_length]], dim=2)
        ###################################################
        ...
        value = rearrange(value, "(b f) d c -> b f d c", f=video_length)
        # value = torch.cat([value[:,  [0] * video_length], value[:, former_frame_index]], dim=2)
        value = torch.cat([value[:,  [0] * video_length], value[:, [1] * video_length]], dim=2)
        value = rearrange(value, "b f d c -> (b f) d c")
