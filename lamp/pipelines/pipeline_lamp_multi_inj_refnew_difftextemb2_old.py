...
class LAMPPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    @torch.no_grad()
    def images2embeddings(self, images, dtype, high_freq_percentage=None, low_freq_percentage=None):
        """
        Convert RGB image to VAE latents
        """
        device = self._execution_device
        ...
        print(images.dtype)
        images = rearrange(images, "f h w c -> f c h w").to(device)
        images_freq, _ = split_fft_values(images, torch.ones(1)*2, high_freq_percentage, low_freq_percentage)
        images_freq = F.interpolate(images_freq, (224, 224), mode='bilinear')#(b*2, 3, h, w)
        print(images_freq.dtype)
        freq_embeddings = self.image_encoder(images_freq.to(dtype), output_hidden_states=True)[0]#(b*2, l, c)
        image_embeddings = combine_embeddings(freq_embeddings, flag_to_combine=torch.ones(1)*2)
        inj_embeddings = self.mapper([image_embeddings])
        return inj_embeddings
    
    @torch.no_grad()
    def freq2embeddings(self, freq_embeddings, bsz, dtype):
        freq_embeddings_input = torch.repeat_interleave(freq_embeddings.unsqueeze(1), bsz, 0)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Encode input prompt
        # inj_embeddings = self.images2embeddings(
        #     np.array(Image.open(ref_img_path[0]).convert('RGB').resize((width, height)))[None, :], 
        #     torch.float32, 
