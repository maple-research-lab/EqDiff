...
class StableDiffusionPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        reference_encoder: AppearanceEncoderModel,
        scheduler: Union[
            DDIMScheduler,
            PNDMScheduler,
            LMSDiscreteScheduler,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if controlnet_cond is not None:
                    ...
                    controlnet_latent_input = rearrange(controlnet_latent_input, 'b c f h w -> (b f) c h w').clone()
                    ...
                    for sample in down_block_res_samples:
                        sample = rearrange(sample, '(b f) c h w -> b c f h w', b=1, f=video_length)
                        _down_block_res_samples.append(sample)
                    down_block_res_samples = _down_block_res_samples
                    mid_block_res_sample = rearrange(mid_block_res_sample, '(b f) c h w -> b c f h w', b=1, f=video_length)

                # referene encoder inference
                if self.reference_encoder is not None:
                    ref_text_embeddings = text_embeddings.repeat_interleave(num_objects, 0)
